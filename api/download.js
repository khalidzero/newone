...
// Helper function to check if a string looks like HTML
...
function callRapidApi(videoUrl) {
  return new Promise((resolve, reject) => {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          try {
            ...
            if (isHtmlResponse(responseText)) {
              reject(new Error('L\'API a retourné une page HTML au lieu de JSON. Veuillez réessayer plus tard.'));
              return;
            }
            ...
            // If we can't parse the response as JSON, include part of the raw response in the error
            reject(new Error(`Erreur lors de l'analyse de la réponse de l'API: ${responseText.substring(0, 100)}...`));
          }
        } else {
          // Handle API error response
          try {
            // Check if error response looks like HTML
            if (isHtmlResponse(responseText)) {
              reject(new Error(`L'API a retourné une page HTML d'erreur (code ${response.statusCode}). Veuillez réessayer plus tard.`));
              return;
            }
            ...
            reject(new Error(`L'API a retourné une erreur non-JSON (code ${response.statusCode}): ${responseText.substring(0, 100)}...`));
          }
        }
      });
    });
    ...
    req.setTimeout(10000, () => {
      req.abort();
      reject(new Error('La requête à l\'API a expiré après 10 secondes'));
    });
